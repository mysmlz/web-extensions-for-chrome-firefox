...
window.onload = () => {
    ...
    const _interval: number = setInterval(() => {
...
function setOption(wrapper: HTMLDivElement, elements: Array<HTMLElement> | NodeListOf<HTMLElement>, id: string, innerHTML: string, title: string,
    ...
    option.onclick = () => {
        ...
        try {
            //@ts-ignore
            browser.storage.local.set({'bling-config': JSON.stringify(config)}).then(() => {
            }, () => {
                console.error("error");
            });
        } catch (e) {
...
function background() {
    if (location.href.match(/[\s\S]+com\/video\//)) {
        ...
        body.onwheel = function () {
            ...
            const liveCard: NodeListOf<HTMLElement> = document.querySelectorAll('.bili-live-card');
    ...
    let time1: number = 0;
    const interval1: number = setInterval(() => {
        if (time1 > times) {
    ...
    let time3: number = 0;
    const noUploadClass: string = 'bling-no-upload';
    const values: Array<string> = ['动态', '收藏', '历史'];
    const interval3: number = setInterval(() => {
        if (time3 > times) {
            clearInterval(interval3);
        }
        let elements: Array<HTMLElement> = [];
        document.querySelectorAll('.right-entry .v-popover-wrap').forEach((element: HTMLElement, index: number) => {
            const text: string = element.querySelector('.right-entry__outside span')?.innerHTML.trim();
            if (values.indexOf(text) >= 0) {
                element.classList.add(`bling-no-upload-${index}`);
    ...
    let time4: number = 0;
    const interval4: number = setInterval(() => {
        if (time4 > times) {
            clearInterval(interval4);
        }
        //无"充电"按钮时,延长"关注"按钮,使其更美观
    ...
    let time5: number = 0;
    const interval5: number = setInterval(() => {
        if (time5 > times) {
            clearInterval(interval5);
        }
        const nickname: HTMLAnchorElement = document.querySelector('.nickname-item');
        const info__name: HTMLDivElement = document.querySelector('.info__name');
        if (nickname !== null) {
            if (config.founder) {
    ...
    let time6: number = 0;
    const interval6: number = setInterval(() => {
        if (time6 > times) {
